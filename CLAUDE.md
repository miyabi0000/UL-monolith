...
- **横並びコンポーネントは必ず `items-center` を付け、子要素の高さは `CONTROL_HEIGHT`（badge:20 / base:36 / lg:44）のいずれかに揃える。高さ混在禁止。** `ROW_CLASSES` / Tailwind の `h-control` / `h-control-lg` / `h-badge` を活用する
