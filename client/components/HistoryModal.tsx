...
const HistoryModal: React.FC<HistoryModalProps> = ({ 
  ...
  const getActionColor = (action: string) => {
    switch (action) {
      ...
      case 'bulk_update': return 'text-purple-600 bg-purple-50';
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(80vh-120px)]">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="animate-spin h-8 w-8 border-2 border-gray-400 dark:border-gray-600 border-t-transparent rounded-full"></div>
              <span className="ml-2 text-gray-600 dark:text-gray-400">履歴を読み込み中...</span>
            </div>
          ) : history.length === 0 ? (
                ...
                <div key={entry.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {entry.action !== 'delete' && entry.action !== 'bulk_delete' && onRevert && (
                      <button
                        ...
                        className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 px-2 py-1 rounded"
                      >
                        この版に戻す
                      </button>
                    )}
                  ...
                  <div className="space-y-2">
                    {entry.changes.map((change, changeIndex) => (
                      <div key={changeIndex} className="bg-gray-50 dark:bg-gray-800 p-3 rounded">
                        <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                          {change.field}
                        </div>
                        <div className="flex items-center space-x-2 text-sm">
                          <span className="text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded">
