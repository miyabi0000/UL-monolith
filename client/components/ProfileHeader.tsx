  ...
  <section className="rounded-2xl neu-raised bg-white/75 backdrop-blur dark:bg-slate-800/70 overflow-hidden">
    ...
    <div className="p-4 flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex items-center gap-1.5">
        {/* ログイン/ログアウト（モバイルでのみ表示、sm以上はAppDockに委譲） */}
        {onShowLogin && !isAuthenticated && (
          <button
            type="button"
            onClick={onShowLogin}
            className="sm:hidden h-8 px-3 rounded-lg text-xs font-medium text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"
          >
            Login
          </button>
        )}
